...
// --- FUNGSI BARU UNTUK PERBANDINGAN VERSI HEKSADESIMAL ---
function firmware_version_compare($ver1, $ver2) {
    // Ambil 4 karakter terakhir sebagai bagian yang berpotensi berubah
    $suffix1 = substr($ver1, -4);
    $suffix2 = substr($ver2, -4);

    // Ambil bagian utama dari versi
    $base1 = substr($ver1, 0, -4);
    $base2 = substr($ver2, 0, -4);

    // Jika bagian utama berbeda, gunakan perbandingan string biasa
    if ($base1 !== $base2) {
        return strcmp($base1, $base2);
    }
    
    // Jika bagian utama sama, bandingkan 4 karakter terakhir
...
// Logika untuk reset (mengosongkan tabel hasil dan textarea)
...
// Gunakan session untuk menyimpan data textarea
...
if (isset($_POST['action']) && $_POST['action'] === 'compare') {
    ...
    if (!empty($base_text) && !empty($comparison_text) && !empty($reference_map)) {
        ...
        foreach ($base_data as $row) {
            ...
            if (!isset($base_map[$key]) || firmware_version_compare($row['ap_version'], $base_map[$key]['ap_version']) > 0) {
                $base_map[$key] = $row;
            }
        }
        ...
        foreach ($comparison_data as $row) {
            $key = $row['model_name'];
            if (!isset($comparison_map[$key]) || firmware_version_compare($row['ap_version'], $comparison_map[$key]['ap_version']) > 0) {
        ...
        foreach ($comparison_map as $model_name => $comp_row) {
            if (isset($reference_map[$model_name])) {
                ...
                if (isset($base_map[$lookup_key])) {
                    ...
                    if (firmware_version_compare($base_row['ap_version'], $comp_row['ap_version']) > 0) {
                        // MODIFIKASI 1: Tambahkan old_cp dan old_csc saat menyimpan hasil
                        $result_row = [
                            ...
                            'old_cp'          => $comp_row['cp_version'],
                            'old_csc'         => $comp_row['csc_version'],
        ...
        if (!empty($current_run_results)) {
            // MODIFIKASI 2: Update query INSERT dan UPDATE untuk menyertakan kolom baru
            $stmt = $pdo->prepare("
                INSERT INTO comparison_result (model_name, customer_base, new_ap, old_ap, cp, csc, old_cp, old_csc, full_row_string)
                VALUES (:model_name, :customer_base, :new_ap, :old_ap, :cp, :csc, :old_cp, :old_csc, :full_row_string)
                ON DUPLICATE KEY UPDATE
                    new_ap = VALUES(new_ap), old_ap = VALUES(old_ap), cp = VALUES(cp),
                    csc = VALUES(csc), old_cp = VALUES(old_cp), old_csc = VALUES(old_csc), 
                    full_row_string = VALUES(full_row_string), comparison_date = NOW()
            ");
...
// Ambil semua hasil dari database dengan sorting kustom
$comparison_results = $pdo->query("SELECT * FROM comparison_result ORDER BY SUBSTRING(model_name, 4) ASC")->fetchAll();
...
// Urutkan juga hasil baru untuk clipboard
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <label for="base_data" class="block font-semibold text-slate-600 dark:text-slate-300">Tabel BASE (Referensi: OXM, OLM, dll)</label>
        ...
        <div class="mt-12">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                ...
                <div class="flex items-center gap-2">
                    <a href="export.php" class="px-5 py-2 text-sm font-medium text-white bg-teal-500 hover:bg-teal-600 rounded-full shadow-lg shadow-teal-500/30 transition-all">Export ke Excel</a>
                ...
                <table class="w-full text-sm text-left">
                    <thead class="bg-slate-100/50 dark:bg-slate-800/50 text-xs uppercase">
                        <tr>
                            ...
                            <th class="px-4 py-3">Type Release</th>
                            <th class="px-4 py-3">Base AP Version</th>
                            <th class="px-4 py-3">XID AP Version</th>
                        ...
                        <tr class="hover:bg-slate-200/30 dark:hover:bg-slate-800/30 transition-colors">
                            ...
                            <td class="px-4 py-3 align-middle">
                                <?php
                                    $release_type = '';
                                    $bg_color = '';
                                    $ap_version = $row['new_ap'];

                                    if (strpos($ap_version, 'XXU') !== false || strpos($ap_version, 'DXU') !== false) {
                                        $release_type = 'Maintenance Release';
                                        $bg_color = 'bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300';
                                    } elseif (strpos($ap_version, 'XXS') !== false || strpos($ap_version, 'DXS') !== false) {
                                        $release_type = 'Security MR';
                                        $bg_color = 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300';
                                    }
                                ?>
                                <?php if ($release_type): ?>
                                    <span class="px-2.5 py-1 text-xs font-semibold rounded-full <?= $bg_color ?>">
                                        <?= $release_type ?>
                                    </span>
                                <?php endif; ?>
                            </td>
                            <td class="px-4 py-3 font-mono align-top">
                                <span class="px-2 py-0.5 text-xs font-bold text-green-800 dark:text-green-200 bg-green-100 dark:bg-green-900/50 rounded-full">
                                    <?= htmlspecialchars($row['new_ap']) ?>
                                </span>
                            ...
                            <td class="px-4 py-3 font-mono align-top">
                                <span class="px-2 py-0.5 text-xs font-medium text-red-800 dark:text-red-300 bg-red-100 dark:bg-red-900/50 rounded-full">
                                    <?= htmlspecialchars($row['old_ap']) ?>
                                </span>
                                <div class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">
                                    <div>CP: <?= htmlspecialchars($row['old_cp']) ?></div>
                                    <div>CSC: <?= htmlspecialchars($row['old_csc']) ?></div>
                                </div>
                            </td>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fallbackCopy = (text, button) => {
        ...
        try {
            if (document.execCommand('copy')) showSuccess(button);
        } catch (err) {
    ...
    const showSuccess = (button) => {
        ...
        if (button.id === 'copyNewBtn' || button.id === 'copyAllBtn') {
             ...
             button.classList.remove('bg-green-500', 'bg-sky-200/50', 'dark:bg-sky-800/50');
    ...
    if (copyNewBtn) {
        copyNewBtn.addEventListener('click', () => copyToClipboard(document.getElementById('clipboard-new').value, copyNewBtn));
    }

    const copyAllBtn = document.getElementById('copyAllBtn');
    if(copyAllBtn) {
        copyAllBtn.addEventListener('click', () => copyToClipboard(document.getElementById('clipboard-all').value, copyAllBtn));
    }

    document.querySelectorAll('.copy-row-btn').forEach(button => {
        button.addEventListener('click', () => copyToClipboard(button.dataset.clipboardText, button));
    });
});
</script>
